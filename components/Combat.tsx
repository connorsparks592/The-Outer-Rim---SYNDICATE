...
import { Sword, Skull, Backpack, ArrowRight, Activity, Zap } from 'lucide-react';
                ...
                <AnimatePresence>
                    <motion.div 
                        ...
                        animate={{ 
                            ...
                            filter: isPlayerAttacking ? 'brightness(3) contrast(2)' : 'brightness(1) contrast(1)'
                        }}
        ...
        <div className="flex-1 flex flex-col p-4 md:p-6 bg-red-950/5 border-t border-red-500/10 h-full overflow-hidden">
            <div className="flex items-center gap-3 mb-6">
                ...
                <div className="min-w-0">
                    <h2 className="text-white font-title text-[10px] tracking-[0.3em] uppercase truncate">Combat Tactical Override</h2>
                    <p className="text-[8px] text-red-500/60 font-mono uppercase tracking-[0.2em]">Target: {enemy.name}</p>
                </div>
            </div>
            ...
            <div className="flex-1 grid grid-cols-2 gap-4 h-full min-h-0">
                ...
                <div className="grid grid-cols-1 gap-3">
                    ...
                    >
                        <Sword size={24} className={playerCooldown >= 100 ? "animate-bounce" : ""} />
                        <span className="font-title text-[10px] tracking-widest uppercase mt-2">Blast Target</span>
                        <div 
                    ...
                    >
                        <Backpack size={16} />
                        <span className="font-title text-[10px] tracking-[0.2em] uppercase">Tactical Gear</span>
                    </button>
                    ...
                    >
                        <ArrowRight size={16} />
                        <span className="font-title text-[10px] tracking-[0.2em] uppercase">Withdraw</span>
                    </button>
                </div>
                        ...
                        <div className="flex items-center gap-3">
                            ...
                            <div className="flex-1">
                                <div className="flex justify-between items-center mb-1">
                                    <span className="text-[8px] text-red-500 font-title tracking-widest">Enemy Momentum</span>
                                    <span className="text-[10px] font-mono text-red-400">{Math.floor(enemyCooldown)}%</span>
                                </div>
                ...
                {showInventory && (
                    ...
                    >
                        <div className="flex justify-between items-center mb-4">
                            <h3 className="text-cyan-400 font-title text-[10px] tracking-[0.3em] uppercase">Field Inventory</h3>
                            <button onClick={() => setShowInventory(false)} className="text-cyan-400 text-[10px] tracking-widest uppercase hover:underline">Dismiss</button>
                        </div>
                        <div className="flex-1 space-y-2 overflow-y-auto no-scrollbar">
                            {gameState.inventory.filter(i => i.type === 'consumable').length === 0 && (
                                <p className="text-gray-600 text-xs text-center italic py-4">No tactical supplies.</p>
                            )}
                            {gameState.inventory.filter(i => i.type === 'consumable').map((item, idx) => (
                                <button 
                                    key={idx}
                                ...
                                >
                                    <div className="flex flex-col text-left">
                                        <span className="text-white font-title text-xs uppercase">{item.name}</span>
                                        <span className="text-[8px] text-green-500 uppercase tracking-widest">Regen +{item.heal} HP</span>
                                    </div>
                                    <span className="bg-cyan-500/20 text-cyan-400 px-2 py-0.5 rounded text-[10px] font-title">x{item.count}</span>
                                </button>
                            ))}
                        </div>
                    </motion.div>
