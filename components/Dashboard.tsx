...
import { Character, Quest, Item, SaveData, Locations, Location, DialogueNode } from '../types';
...
export const Dashboard: React.FC<DashboardProps> = ({ gameState, setGameState, locations, onSave }) => {
    ...
    const [activeMinigame, setActiveMinigame] = useState<'slicing' | 'sabacc' | 'podracing_bet' | 'podracing_tracker' | 'shooting_range' | 'salvage' | 'holonet' | null>(null);
    const [isMobileMenuOpen, setIsMobileMenuOpen] = useState(false);
    const [isCharacterSheetOpen, setIsCharacterSheetOpen] = useState(false);
    const [slicingTarget, setSlicingTarget] = useState<string | null>(null);
    const [invSort, setInvSort] = useState<'A-Z'|'Z-A'|'High-Low'|'Rarity'>('A-Z');
    const [invFilter, setInvFilter] = useState<'all'|'weapon'|'consumable'|'misc'|'furniture'|'utility'>('all');
        ...
        const timer = setInterval(() => {
            ...
            if (gameState.activeSlicingTask) {
                ...
                if (elapsed >= (gameState.activeSlicingTask.duration * 1000)) {
                    ...
                    addToLog(`[RTC NOTIFICATION] Decryption complete: Access to ${target} granted.`);
                    setLogKey(k => k + 1);
                }
            }
        }, 1000);
    ...
    const triggerEnemyAttack = () => {
        if (!combatEnemy) return;
        const dmg = Math.max(1, combatEnemy.dmg + Math.floor(Math.random() * 3) - 1);
    ...
    const handleCombatLoss = () => {
        setCombatEnemy(null);
        addToLog("[COMBAT] You were defeated. Transporting to nearby Med Center...");
        setGameState(prev => ({
            ...prev,
            currentLocationId: 'med_center',
            stats: prev.stats ? { ...prev.stats, currentHp: Math.floor(prev.stats.maxHp * 0.1) } : null
        }));
    };
        ...
        setGameState(prev => {
            ...
            let newState = {
                ...
                quests: prev.quests.map(q => {
                    if (q.id === 'q2' && enemyId === 'teemo') return { ...q, currentStepIndex: 4 };
                    if (q.id === 'q3' && enemyId === 'thug' && q.currentStepIndex === 2) return { ...q, currentStepIndex: 3 };
                    return q;
                })
            };

            // Contract Check
            const activeContractIdx = newState.activeContracts.findIndex(c => c.targetLocation === currentLocation.name && !c.completed);
            if (activeContractIdx > -1) {
                const contract = newState.activeContracts[activeContractIdx];
                newState.credits += contract.reward;
                newState.activeContracts = newState.activeContracts.filter((_, i) => i !== activeContractIdx);
                addToLog(`[CONTRACT COMPLETE] Target eliminated. Reward: ${contract.reward} Credits synced.`);
            }
            ...
            if (enemyId === 'thug' && newState.currentLocationId === 'mos_eisley_old_quarter') {
                 // Sample quest progression mapping
                 newState.quests = newState.quests.map(q => {
                     if (q.id === 'q_tutorial' && q.currentStepIndex === 1) return { ...q, currentStepIndex: 2 };
                     return q;
                 });
            }
            ...
            if (enemyId === 'teemo') {
                newState.quests = newState.quests.map(q => q.id === 'q2' ? { ...q, status: 'completed', currentStepIndex: 4 } : q);
    ...
    const handlePlayerAttack = () => {
        ...
        const totalDmg = baseDmg + Math.floor(Math.random() * (gameState.stats?.stats.str || 5));
    ...
    const locName = isNight ? (currentLocation.nightName || currentLocation.name) : currentLocation.name;
    const locDesc = isNight ? (currentLocation.nightDescription || currentLocation.description) : currentLocation.description;
    
    // Switch to NPC image if talking
    const locImg = activeNpc 
        ? (activeNpc.imageUrl) 
        : (isNight ? (currentLocation.nightImageUrl || currentLocation.imageUrl) : currentLocation.imageUrl);

    const locExits = isNight ? (currentLocation.nightExits || currentLocation.exits) : currentLocation.exits;
    const locNpcs = isNight ? (currentLocation.nightNpcs || currentLocation.npcs || []) : (currentLocation.npcs || []);
    const locActions = isNight ? (currentLocation.nightActions || currentLocation.actions || []) : (currentLocation.actions || []);
    
    // Furniture/Utility Actions in Safehouse
    const isSafehouse = gameState.currentLocationId === 'safehouse' || gameState.currentLocationId === 'safehouse_bedroom';
    const furnitureItems = isSafehouse ? gameState.inventory.filter(i => (i.type === 'furniture' || i.type === 'utility') && i.service) : [];
    
    const locSearchables = isNight ? (currentLocation.nightSearchables || currentLocation.searchables || []) : (currentLocation.searchables || []);
    ...
    const generateContract = () => {
        const targets = ["Gredda the Hutt", "Vexon the Sly", "Dune Sea Wanderer", "Imperial Defector", "Smuggler Jax", "Renegade Droid"];
        const locs = ["Jundland Wastes", "Mos Eisley Old Quarter", "Krayt Valley", "Mos Espa", "Anchorhead", "Tosche Station"];
    ...
    const handleContractAction = () => {
        ...
        addToLog(`[GUILD TERMINAL] Contract assigned: ${newContract.title}. Target: ${newContract.targetLocation}.`);
    };

    const checkContractCompletion = (locationName: string) => {
        const contract = gameState.activeContracts.find(c => c.targetLocation === locationName && !c.completed);
        if (contract) {
    ...
    const handleMove = async (exitId: string) => {
        ...
        if (nextLoc.reqQuestState) {
            const quest = gameState.quests.find(q => q.id === nextLoc.reqQuestState!.id);
            const isCompleted = quest?.status === 'completed';
            const hasStep = quest && (quest.status === 'completed' || quest.currentStepIndex >= nextLoc.reqQuestState!.step);
            
            const requirementMet = nextLoc.reqQuestState.completed ? isCompleted : hasStep;
            
            if (!requirementMet) {
                addToLog(`[SECURITY ERROR] Access Denied. Sector ${nextLoc.name} is currently restricted.`);
                if (nextLoc.reqQuestState.completed) {
                    addToLog(`Clearance code from "${quest?.title || 'Unknown Protocol'}" required.`);
                } else {
                    addToLog(`Further progress in "${quest?.title || 'Unknown Protocol'}" needed.`);
                }
                return;
            }
        }
        ...
        if (nextLoc.reqItem) {
            ...
            if (!hasItem) {
                ...
                addToLog(`[HARDWARE ERROR] Link failure. ${item?.name || 'Required Hardware'} required to interface with ${nextLoc.name}.`);
                return;
            }
        }
        ...
        if (gameState.currentLocationId.includes('safehouse') && exitId === 'swoop_garage') {
            if ((gameState.reputation['kaelenTalkedTo'] || 0) < 1) {
                addToLog("Kaelen: 'Wait! Don't leave yet. We need to talk about our setup here.'");
        ...
        setGameState(prev => {
            ...
            return {
                ...
                visitedLocations: visited ? prev.visitedLocations : [...prev.visitedLocations, exitId],
                history: ["System Initialized.", `Location: ${nextLoc.name}.`]
            };
        });

        // Local arrival log
        const desc = isNight ? (nextLoc.nightDescription || nextLoc.description) : nextLoc.description;
        addToLog(desc);
    };
    ...
    const handleCheatSubmit = (e: React.FormEvent) => {
        ...
        if (code === 'motherlode') {
            ...
            addToLog("ADMIN: Data corruption detected in bank records... +5000 Credits synced.");
        } else if (code === 'refill') {
            setGameState(prev => ({
                ...prev,
                stats: prev.stats ? { ...prev.stats, stats: { ...prev.stats.stats, vit: 100 } } : null
            }));
            addToLog("ADMIN: Vitality levels force-recalibrated to 100%.");
        } else if (code === 'heist') {
            const legendaries = ITEM_DATABASE.filter(i => i.rarity === 'legendary');
            const randomItem = legendaries[Math.floor(Math.random() * legendaries.length)];
            setGameState(prev => ({
                ...prev,
                inventory: [...prev.inventory, { ...randomItem, count: 1 }]
            }));
            addToLog(`ADMIN: Manifest override. ${randomItem.name} added to inventory.`);
        } else {
            addToLog(`ADMIN: Invalid access code "${code}". Trace protocol initiated.`);
        }
        setCheatCode('');
    };
    ...
    const handleRest = async () => {
        ...
        addToLog("System recalibration complete. Vitality levels at 100%. All systems nominal.");
    };
            ...
            if (!item.lastUsedAt || (now - item.lastUsedAt) > (12 * 3600 * 1000)) {
                if (item.buffEffect && gameState.stats) {
                    ...
                    setGameState(prev => ({
                        ...
                        inventory: prev.inventory.map(i => i.id === item.id ? { ...i, lastUsedAt: now } : i)
                    }));
                    addToLog(`Interactive: Used ${item.name}. ${item.description}`);
                    addToLog(`STATUS BOOST: +${value} ${stat.toUpperCase()} applied.`);
                }
            } else {
                const remaining = Math.ceil((12 * 3600 * 1000 - (now - item.lastUsedAt)) / 3600000);
    ...
    const handleAction = (action: string) => {
        if (action === 'Look Around') {
            const detailed = isNight ? (currentLocation.nightDetailedDescription || currentLocation.detailedDescription) : currentLocation.detailedDescription;
            ...
            if (currentLocation.encounters) {
                ...
                if (encounter) {
                    ...
                    if (encounter.lootDescription && encounter.lootLabel) {
                         ...
                         if (encounter.item) {
                             const itemObj = ITEM_DATABASE.find(i => i.id === encounter.item);
                             if (itemObj) {
                                 setGameState(prev => ({ ...prev, inventory: [...prev.inventory, itemObj] }));
            ...
            if (gameState.activeSlicingTask) {
                addToLog("A slicing task is already in progress. Link is currently saturated.");
                return;
            }
            ...
            addToLog("Decryption Protocol engaged. Estimated time: 60 seconds. Background process running...");
        }
            ...
            addToLog("Storage is currently empty. You can store items here in a future update.");
        }
            ...
            if (gameState.credits >= cost) { 
                setGameState((prev:any) => ({
                    ...
                    stats: prev.stats ? { ...prev.stats, currentHp: Math.min(prev.stats.maxHp, prev.stats.currentHp + 5) } : null
                })); 
                addToLog(`You enjoy a refreshing drink. ${cost === 2 ? '(Happy Hour Discount Applied!)' : ''}`); 
            } else { 
             ...
             if (gameState.lastStipendClaimDate !== today) {
                 ...
                 addToLog("You have already collected your stipend for today. Come back tomorrow.");
             }
        }
            ...
            if (gameState.activeRace && !gameState.activeRace.completed) {
                if (gameState.activeRace.type === 'rookie' && gameState.activeRace.startTime === 0) {
                    setGameState(prev => ({
                        ...prev,
                        activeRace: prev.activeRace ? { ...prev.activeRace, startTime: Date.now() + 5000 } : null
                    }));
                }
                setActiveMinigame('podracing_tracker');
            } else if (gameState.activeRace && gameState.activeRace.completed) {
                addToLog("The previous race has finished. Place a new bet to watch another.");
            } else {
                addToLog("There is no active race at the moment. Place a bet at the terminal to trigger a Rookie event.");
            }
        }
        else if (action === 'Range Practice') {
            setActiveMinigame('shooting_range');
            addToLog("Entering the Bounty Guild's training range... Synchronizing simulator.");
        }
            ...
            if (q6 && q6.currentStepIndex === 3) {
                setGameState(prev => ({
                    ...prev,
                    quests: prev.quests.map(q => {
                        if (q.id === 'q6') return { ...q, status: 'completed', currentStepIndex: 4 };
                        if (q.id === 'q_endless') return { ...q, status: 'active' };
                        return q;
                    })
                }));
                addToLog("CREDENTIALS ACCEPTED. You are now a permanent resident of Mos Eisley. The Empire recognizes your status—for a price.");
                addToLog("MAIN QUEST COMPLETE: You have survived and established a foothold on Tatooine.");
                addToLog("NEW QUEST ACTIVE: Outer Rim Legend (Endless Mode unlocked).");
            } else {
                addToLog("ACCESS DENIED. Indefinite residency requires active reputation data (Complete previous quest steps).");
            }
        }
        else if (action === 'Manage Residence') {
             addToLog("Managing Safehouse Inventory... System ready.");
             const furniture = gameState.inventory.filter(i => i.type === 'furniture' || i.type === 'utility');
             if (furniture.length > 0) {
                 addToLog(`Active Systems: ${furniture.map(f => f.name).join(', ')}.`);
             } else {
                 addToLog("Residence is currently at baseline. Acquire furniture to unlock new interactions.");
             }
        }
    };
    ...
    const handleSearch = (searchableId: string) => {
        ...
        if (target.reqQuestState) {
            const quest = gameState.quests.find(q => q.id === target.reqQuestState!.id);
             ...
             setGameState(prev => ({
                 ...
                 quests: prev.quests.map(q => {
                     if (q.id === target.questUpdate!.id) {
                         const updatedQ = { ...q, currentStepIndex: target.questUpdate!.step };
                         if (target.questComplete) {
                             updatedQ.status = 'completed';
                             // Mark all steps as complete
                             updatedQ.steps = updatedQ.steps.map(s => ({ ...s, completed: true }));
                         }
                         return updatedQ;
                     }
                     return q;
        ...
        if (target.startQuest) {
            setGameState(prev => ({
                ...prev,
                quests: prev.quests.map(q => q.id === target.startQuest ? { ...q, status: 'active', currentStepIndex: 1 } : q)
            }));
            const quest = gameState.quests.find(q => q.id === target.startQuest);
            addToLog(`New Quest: ${quest?.title || 'Unknown Protocol'}`);
        }

        setGameState(prev => ({ ...prev, lootedContainers: [...prev.lootedContainers, searchableId] }));
    };
    ...
    const startDialogue = (npcId: string) => {
        ...
        const greetingId = (isNight && npc.nightGreetingId) ? npc.nightGreetingId : npc.greetingId;
    ...
    const handleDialogueOption = (opt: any) => {
        if (opt.action) {
            ...
            const gameProxy = {
                ...
                setCredits: (fn: any) => setGameState(prev => ({ ...prev, credits: fn(prev.credits) })),
                addToLog: addToLog,
                updateQuest: (qid: string, step: number) => setGameState(prev => ({ ...prev, quests: prev.quests.map(q => q.id === qid ? { ...q, currentStepIndex: step } : q) })),
                completeQuest: (qid: string) => setGameState(prev => ({ ...prev, quests: prev.quests.map(q => q.id === qid ? { ...q, status: 'completed', steps: q.steps.map(s => ({ ...s, completed: true })) } : q) })),
                setInventory: (fn: any) => setGameState(prev => ({ ...prev, inventory: fn(prev.inventory) })),
                setStats: (fn: any) => setGameState(prev => ({ ...prev, stats: fn(prev.stats) })),
                setQuests: (fn: any) => setGameState(prev => ({ ...prev, quests: fn(prev.quests) })),
                setDefeatedNpcs: (fn: any) => setGameState(prev => ({ ...prev, defeatedNpcs: fn(prev.defeatedNpcs) })),
                updateReputation: (npcId: string, amount: number) => setGameState(prev => ({ ...prev, reputation: { ...prev.reputation, [npcId]: (prev.reputation[npcId] || 0) + amount } })),
    ...
    const renderMinigame = () => {
            ...
            return (
                <SlicingGame 
                    ...
                    onWin={() => {
                        ...
                        setGameState(prev => {
                            const newState = { ...prev, unlockedContainers: [...prev.unlockedContainers, slicingTarget!] };
        ...
        if (activeMinigame === 'podracing_bet' || activeMinigame === 'podracing_tracker') {
            return (
                <Podracing 
                    gameState={gameState}
                    ...
                    onRaceFinished={(winnerId, payout) => {
                        ...
                        if (payout > 0) {
                            addToLog(`RACE UPDATE: Your driver won! You received ${payout} credits.`);
                        } else {
            ...
            return (
                <TargetRange 
                    ...
                    onFinish={(score, credits) => {
                        ...
                        addToLog(`Training session concluded. Final Score: ${score}. Credits Synced: +${credits}.`);
                    }}
                />
            );
        }
            ...
            return (
                <SalvageGame 
                    ...
                    onFinish={(parts, credits) => {
                        ...
                        addToLog(`Salvage operation complete. ${parts} parts recovered. Credits Earned: +${credits}.`);
                        if (parts >= 4) {
                            ...
                            if (roll < 0.3) {
                                ...
                                if (item) {
                                    setGameState(prev => ({ ...prev, inventory: [...prev.inventory, { ...item, count: 1 }] }));
                                    addToLog("Bonus: You salvaged a functional Power Cell from the debris.");
                                }
                            }
                        }
                    }}
                ...
                <HoloNet 
                    ...
                    onOpenTracker={() => {
                        if (gameState.activeRace && !gameState.activeRace.completed) {
                            ...
                            addToLog("SYSTEM: No live pro signals detected. Proceed to Arena for manual uplink.");
                        }
                    }}
                />
            );
    ...
    const useItem = (item: Item) => {
        if (item.heal) {
            setGameState(prev => {
                if (!prev.stats) return prev;
                const newHp = Math.min(prev.stats.maxHp, prev.stats.currentHp + item.heal!);
                const newInv = prev.inventory.map(i => i.id === item.id ? { ...i, count: i.count - 1 } : i).filter(i => i.count > 0);
                return { ...prev, stats: { ...prev.stats, currentHp: newHp }, inventory: newInv };
            });
    ...
    const isGreedoUnlocked = gameState.quests.some(q => q.id === 'q2' && (q.status === 'active' || q.status === 'completed'));
            ...
            >
                     ...
                     >
                         ...
                         <div className="flex flex-col text-left justify-center pb-0.5">
                             <div className="flex items-center gap-2 mb-0.5">
                                 <span className="text-white font-title text-sm md:text-base tracking-wide uppercase group-hover:text-cyan-400 transition-colors leading-none">{gameState.stats?.name}</span>
                                 <span className="px-1.5 py-0.5 bg-yellow-500/20 text-yellow-500 font-bold text-[8px] rounded-[3px] uppercase border border-yellow-500/30 leading-none shadow-[0_0_8px_rgba(234,179,8,0.2)]">LVL {gameState.stats?.level}</span>
                             </div>
                             <span className="text-[9px] text-cyan-600 font-display tracking-widest uppercase leading-none">{gameState.stats?.charClass?.name || 'Unknown'}</span>
                         </div>
                     </button>
                </div>
                ...
                <div className="flex-1 flex flex-row items-center justify-center gap-4 md:gap-8 w-full md:w-auto order-last md:order-none">
                    <div className="flex flex-col flex-1 max-w-[150px] md:max-w-[200px]">
                        <div className="flex justify-between items-end mb-1">
                            <span className="text-[7px] text-cyan-600 uppercase tracking-widest leading-none">Vitality</span>
                            <span className={cn("font-mono text-[9px] leading-none", (gameState.stats?.currentHp || 0) < 10 ? 'text-red-500' : 'text-green-400')}>
                                {gameState.stats?.currentHp || 0} / {gameState.stats?.maxHp || 100}
                            </span>
                        </div>
                        <div className="h-1 md:h-1.5 bg-black/80 rounded-full overflow-hidden border border-cyan-900/50">
                            <motion.div 
                                ...
                                className={cn("h-full", (gameState.stats?.currentHp || 0) < 10 ? 'bg-red-500 shadow-[0_0_10px_red]' : 'bg-green-500 shadow-[0_0_10px_rgba(34,197,94,0.5)]')}
                            />
                        </div>
                    </div>
                    ...
                    <div className="flex flex-col flex-1 max-w-[150px] md:max-w-[200px]">
                        <div className="flex justify-between items-end mb-1">
                            <span className="text-[7px] text-cyan-600 uppercase tracking-widest leading-none">Experience</span>
                            <span className="text-[9px] font-mono text-white/40 leading-none">{gameState.stats?.xp} / {gameState.stats?.xpToNextLevel}</span>
                        </div>
                        <div className="h-1 md:h-1.5 bg-black/80 rounded-full overflow-hidden border border-white/10">
                             <motion.div 
                                animate={{ width: `${(gameState.stats?.xp! / gameState.stats?.xpToNextLevel!) * 100}%` }}
                ...
                <div className="flex items-center gap-3 md:gap-4 shrink-0 justify-end">
                    {gameState.activeSlicingTask && (
                        <div className="flex items-center gap-1.5 px-2 py-1 bg-red-950/40 border border-red-500/40 backdrop-blur-md rounded-[4px] animate-pulse shrink-0">
                            ...
                            <span className="text-[8px] text-red-300 font-mono tracking-tighter uppercase whitespace-nowrap leading-none mt-[1px]">
                                SLICING: {Math.max(0, Math.ceil(gameState.activeSlicingTask.duration - (realTime.getTime() - gameState.activeSlicingTask.startTime) / 1000))}s
                            </span>
                        </div>
                    )}
                    
                    <div className="hidden xl:flex items-center gap-4 text-[9px] font-mono border-r border-cyan-900/50 pr-5 mr-1 mb-0.5">
                        <span className="text-cyan-700">STR<span className="text-white ml-1.5">{gameState.stats?.stats.str}</span></span>
                        <span className="text-cyan-700">INT<span className="text-white ml-1.5">{gameState.stats?.stats.int}</span></span>
                        <span className="text-cyan-700">CHA<span className="text-white ml-1.5">{gameState.stats?.stats.cha}</span></span>
                    </div>
                    ...
                    <div className="flex items-center gap-1.5 mb-0.5 bg-yellow-500/5 px-2 py-1 rounded border border-yellow-500/20">
                        ...
                        <span className="font-title text-[13px] text-yellow-400 font-medium tracking-wide">{gameState.credits}</span>
                    </div>
                    ...
                    <div className="flex items-center gap-1.5 mb-0.5 text-cyan-500">
                        ...
                        <span className="text-[11px] font-mono tracking-widest mt-[1px]">
                            {realTime.toLocaleTimeString([], { hour: '2-digit', minute: '2-digit', hour12: false })}
                        </span>
                    </div>
                ...
                <div className="h-[30%] min-h-0 relative overflow-hidden border-b-2 border-cyan-500/10 shrink-0">
                    ...
                    <div className="absolute top-2 left-4 md:top-4 md:left-6 flex items-center gap-3">
                        <div className="p-1 px-2 bg-black/60 border border-cyan-500/30 backdrop-blur-md rounded text-cyan-400">
                            ...
                            <span className="text-[8px] md:text-[10px] uppercase font-display tracking-widest">{gameState.isNight ? 'Sector Shadow' : 'Sector Light'}</span>
                        </div>
                    </div>

                    {/* Title Overlay */}
                    <div className="absolute bottom-2 left-4 md:bottom-4 md:left-8 flex flex-col">
                        <h1 className="text-2xl md:text-4xl font-title text-white uppercase tracking-tighter glow-cyan leading-none mb-0.5">{locName}</h1>
                            ...
                            {dialogueNode ? (
                                ...
                                >
                                    <div className="flex items-center gap-2 text-yellow-500 mb-2 font-title uppercase tracking-[0.3em] text-[10px]">
                                        ...
                                        <span>Communication Link: {NPC_DATABASE[currentNpcId!].name}</span>
                                    </div>
                                    <div className="text-white text-sm md:text-lg leading-relaxed italic font-display animate-flicker">
                                        <Typewriter text={`"${dialogueNode.text}"`} textKey={dialogueNode.text} />
                                    </div>
                                </motion.div>
                            ) : (
                                <div className="flex flex-col gap-2 min-h-full">
                                    {gameState.history.map((line, i) => (
                                        <motion.div 
                                            initial={i === gameState.history.length - 1 ? { opacity: 0, x: -10 } : {}}
                                            animate={{ opacity: 1, x: 0 }}
                                            key={`${i}-${logKey}`} 
                                            className={cn(
                                                "break-words leading-relaxed p-2 px-3 rounded-lg border-l-2 font-mono text-xs md:text-sm",
                                                line.startsWith('>') ? 'text-cyan-600 italic border-cyan-900/30 bg-cyan-950/5' : 
                                                line.startsWith('[COMBAT]') ? 'text-red-400 border-red-500 bg-red-950/20' : 
                                                line.startsWith('System') ? 'text-yellow-400 border-yellow-900 bg-yellow-900/10 text-[10px] font-bold uppercase tracking-widest' :
                                                'text-gray-300 border-cyan-500/20 bg-white/5'
                                            )}
                                        >
                                            {i === gameState.history.length - 1 ? <Typewriter text={line} textKey={logKey} /> : line}
                                        </motion.div>
                                    ))}
                                </div>
                            )}
                        ...
                        {combatEnemy ? (
                            ...
                            >
                                <div className="flex items-center gap-3 mb-4">
                                    ...
                                    <h2 className="text-[10px] font-title text-yellow-500 tracking-[0.4em] uppercase">Dialogue Protocols</h2>
                                </div>
                                <div className="flex-1 grid grid-cols-1 md:grid-cols-2 gap-3 overflow-y-auto no-scrollbar pr-2">
                                    {dialogueNode.options.map((opt, idx) => {
                                        if (opt.reqSkill && gameState.stats && (gameState.stats.stats[opt.reqSkill] || 0) < (opt.reqVal || 0)) return null;
                                        if (opt.reqItem && !gameState.inventory.find(i => i.id === opt.reqItem)) return null;
                                        if (opt.reqQuestState) {
                                            const q = gameState.quests.find(q => q.id === opt.reqQuestState!.id);
                                            if(!q || q.currentStepIndex !== opt.reqQuestState!.step) return null; 
                                        }
                                        if (opt.reqReputation) {
                                            const rep = gameState.reputation[opt.reqReputation.id] || 0;
                                            if (rep < opt.reqReputation.min) return null;
                                        }
                                        if (opt.reqTime && ((opt.reqTime === 'day' && isNight) || (opt.reqTime === 'night' && !isNight))) return null;
                                        ...
                                        return (
                                            ...
                                            >
                                                <span className="uppercase font-display text-sm tracking-wide">{opt.label}</span>
                                                <ArrowRightCircle size={18} className="text-yellow-600 opacity-0 group-hover:opacity-100 transition-opacity" />
                                            </motion.button>
                                        );
                                    })}
                                </div>
                            ...
                            >
                                ...
                                <div className="flex-1 p-4 md:p-6 flex flex-col border-r-2 border-cyan-500/10 overflow-hidden box-border">
                                    <div className="flex items-center gap-3 mb-4">
                                        ...
                                        <h2 className="text-[10px] font-title text-indigo-400 tracking-[0.4em] uppercase">Tactical Navigation</h2>
                                    </div>
                                    <div className="flex-1 overflow-y-auto pr-2 no-scrollbar space-y-4">
                                        {/* EXITS */}
                                        <div className="grid grid-cols-1 sm:grid-cols-2 gap-2">
                                            {locExits.filter(exitId => {
                                                ...
                                                if (loc.reqQuestState) {
                                                    const q = gameState.quests.find(q => q.id === loc.reqQuestState!.id);
                                                    const met = loc.reqQuestState.completed ? q?.status === 'completed' : (q && (q.status === 'completed' || q.currentStepIndex >= loc.reqQuestState.step));
                                                    if (!met) isLocked = true;
                                                }
                                                if (loc.reqItem && !gameState.inventory.find(i => i.id === loc.reqItem)) isLocked = true;
                                                    ...
                                                    >
                                                        <div className="flex items-center gap-3">
                                                            <div className={cn(
                                                                ...
                                                                isLocked ? "bg-red-900" : "bg-indigo-900 group-hover:bg-indigo-400"
                                                            )} />
                                                            <span className={cn(
                                                                "font-title text-xs tracking-wide uppercase",
                                                                isLocked ? "text-red-900" : "text-indigo-100"
                                                            )}>
                                                                {loc.name}
                                                                {isLocked && <span className="ml-2 text-[8px] font-mono opacity-50">[LOCKED]</span>}
                                                            </span>
                                                        </div>
                                                        {isLocked ? (
                                                            <Skull size={14} className="text-red-900 ml-2" />
                                                        ) : (
                                                            <ArrowRightCircle size={16} className="text-indigo-900 group-hover:text-indigo-400 transition-all" />
                                                        )}
                                                    </button>
                                                );
                                            })}
                                        ...
                                        {locNpcs.length > 0 && (
                                             <div className="space-y-2">
                                                 <div className="text-[8px] text-gray-500 font-mono tracking-widest uppercase mb-1">Local Signatures Detected</div>
                                                 <div className="grid grid-cols-1 sm:grid-cols-2 gap-2">
                                                     {locNpcs.filter(id => {
                                                         if (gameState.defeatedNpcs?.includes(id)) return false;
                                                         if (id === 'greedo' && !isGreedoUnlocked) return false;
                                                         ...
                                                         return (
                                                             ...
                                                             >
                                                                 <div className="flex items-center gap-3">
                                                                     <User size={14} className={currentNpcId === npcId ? "text-yellow-500" : "text-cyan-600"} />
                                                                     <span className={cn("font-title text-xs tracking-wide uppercase", currentNpcId === npcId ? "text-yellow-100" : "text-cyan-100")}>{npc.name}</span>
                                                                 </div>
                                                                 <MessageSquare size={16} className={currentNpcId === npcId ? "text-yellow-500" : "text-cyan-600 opacity-40 group-hover:opacity-100"} />
                                                             </button>
                                                         );
                                                     })}
                                                 </div>
                                ...
                                <div className="flex-1 p-4 md:p-6 flex flex-col overflow-hidden box-border">
                                    <div className="flex items-center gap-3 mb-4">
                                        ...
                                        <h2 className="text-[10px] font-title text-cyan-400 tracking-[0.4em] uppercase">Systems Interlink</h2>
                                    </div>
                                    ...
                                    <div className="flex-1 flex flex-col gap-2 overflow-y-auto pr-2 no-scrollbar">
                                        <div className="flex flex-wrap gap-2">
                                            {locActions.map(action => (
                                                ...
                                                >
                                                    {action.includes('Look') ? <Search size={12} /> : <Zap size={12} />}
                                                    {action}
                                                </button>
                                            ))}
                                            {furnitureItems.map(item => (
                                                ...
                                                >
                                                    ...
                                                    {item.service === 'podracing_live' && <Tv size={12} />}
                                                    {item.service === 'buff_provider' && <Sparkles size={12} />}
                                                    {item.name}
                                                </button>
                                            ))}
                                            {locSearchables.filter(s => !gameState.lootedContainers.includes(s.id) || s.locked).map(s => (
                                                <button 
                                                    key={s.id} 
                                                    ...
                                                    className={cn(
                                                        ...
                                                        s.locked && !gameState.unlockedContainers.includes(s.id) 
                                                ...
                                                >
                                                    {s.locked && !gameState.unlockedContainers.includes(s.id) ? <X size={12} /> : <Search size={12} />}
                                                    {s.label}
                                                </button>
                                            ))}
                                        </div>
                        ...
                        >
                            <div className="bg-cyan-500 p-6 flex justify-between items-center">
                                <div className="flex items-center gap-3">
                                    ...
                                    <h2 className="text-xl font-title text-black tracking-widest uppercase">Cargo Bay 94</h2>
                                </div>
                                <button onClick={() => setIsMobileMenuOpen(false)} className="text-black hover:scale-110 transition-transform">
                                    <X size={24} />
                                </button>
                            </div>
                            ...
                            <div className="p-8 space-y-8 overflow-y-auto no-scrollbar">
                                <div>
                                    ...
                                    <div className="grid grid-cols-1 gap-3">
                                        {gameState.inventory.length === 0 && <p className="text-gray-600 italic">No cargo found.</p>}
                                        {gameState.inventory
                                            .filter(i => invFilter === 'all' ? true : i.type === invFilter)
                                            .sort((a, b) => {
                                                if (invSort === 'A-Z') return a.name.localeCompare(b.name);
                                                if (invSort === 'Z-A') return b.name.localeCompare(a.name);
                                                if (invSort === 'High-Low') return b.count - a.count;
                                                if (invSort === 'Rarity') {
                                                    const rVals: Record<string, number> = { common: 0, uncommon: 1, rare: 2, legendary: 3 };
                                                    return (rVals[b.rarity] || 0) - (rVals[a.rarity] || 0);
                                                }
                                                return 0;
                                            })
                                            .map((item, idx) => (
                                                <div key={idx} className="flex flex-col bg-white/5 p-4 rounded-xl border border-white/5 group hover:border-cyan-500/30 transition-all">
                                                    <div className="flex justify-between items-start mb-2">
                                                        <div className="flex flex-col">
                                                            <div className="flex items-center gap-2">
                                                                <span className="text-white font-title text-sm tracking-wide">{item.name}</span>
                                                                <span className={cn("text-[9px] uppercase px-1.5 py-0.5 rounded border leading-none font-bold", 
                                                                    item.rarity === 'legendary' ? 'border-yellow-500/50 text-yellow-500 bg-yellow-500/10' :
                                                                    item.rarity === 'rare' ? 'border-purple-500/50 text-purple-400 bg-purple-500/10' :
                                                                    item.rarity === 'uncommon' ? 'border-green-500/50 text-green-400 bg-green-500/10' :
                                                        ...
                                                        <div className="flex items-center gap-3">
                                                            {(item.heal || item.repModifier || item.buffEffect || item.type === 'consumable') && (
                                                                <button 
                                                                    onClick={() => {
                                                                        useItem(item);
                                                                    }}
                                                            ...
                                                            {item.count > 1 && <span className="bg-cyan-500/20 text-cyan-400 px-2 py-0.5 rounded font-mono text-xs font-bold font-title">x{item.count}</span>}
                                                        </div>
                                                    </div>
                                                    {item.description && <p className="text-[10px] text-gray-400 italic mt-1 leading-snug">{item.description}</p>}
                                                </div>
                                            ))}
                                    </div>
                                </div>
                                ...
                                <div>
                                    <h3 className="text-yellow-500 font-display text-[10px] tracking-[0.4em] uppercase mb-4 border-b border-yellow-900 pb-2">Active Protocols</h3>
                                    <div className="space-y-4">
                                        {gameState.quests.filter(q => q.status === 'active').length === 0 && <p className="text-gray-600 italic">Static connection.</p>}
                                        {gameState.quests.filter(q => q.status === 'active').map(q => (
                                            <div key={q.id} className="p-5 border-l-4 border-yellow-500 bg-yellow-500/5 rounded-r-2xl">
                                                <div className="text-yellow-400 font-title text-sm uppercase tracking-widest mb-1">{q.title}</div>
                                                <p className="text-gray-400 text-xs leading-relaxed font-display">{q.steps[q.currentStepIndex].description}</p>
                                            </div>
                                        ))}
                                    </div>
                                </div>
                                ...
                                <div className="pt-4 border-t border-cyan-900/40">
                                    <h3 className="text-cyan-800 font-display text-[8px] tracking-[0.4em] uppercase mb-3">System Access Console</h3>
                                    <form onSubmit={handleCheatSubmit} className="relative">
                                        <span className="absolute left-3 top-1/2 -translate-y-1/2 text-[10px] font-mono text-cyan-800">USR&gt;</span>
                                        <input 
                                            type="text"
                                            value={cheatCode}
